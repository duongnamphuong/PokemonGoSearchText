...
<?php
    ...
    for ($i = 0; $i < count($lines); $i++) {
        if (preg_match("/(\d+)\t(.+)\t(.+)/", $lines[$i])) {
            $parts = preg_split("/\t/", $lines[$i]);
            $move = new ChargeMove($parts[0], $parts[1]);
            ...
            while ($j < count($lines) and preg_match("/(.+)\t[01]/", $lines[$j])) {
                ...
                $script .= 'insert into PokeChargeMove(PokeId,FormId,MoveId,IsTM)values '.'('.$poke[array_keys($poke)[0]]->getId().','.$poke[array_keys($poke)[0]]->getFormId().','.$move->getId().','.$IsTM.')'."\r\n";
                $count++;
                $j++;
            }
    ...
    echo count($moves)." charge moves and ".$mappingCount." mappings\n";
    
    // write script to file
    $fp = fopen('charge move.sql','w');
    fwrite($fp, $script);
    fclose($fp);
